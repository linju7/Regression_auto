...
def fill_field(page):
    ...
    # 직급명 입력 
    page.locator('input.lw_input[placeholder="직급"]').last.fill(f"준일_자동화_직급_{timestamp}")
...
def access_create_level(page) :
    
    # 수정 버튼 선택 
    page.locator('button.lw_btn:text-is("수정")').click()
    
    # 직급 추가
    page.locator('button.btn_add_row:text-is("직급 추가")').click()
...
def create_level(page, instance, server) :
    url = "admin.worksmobile.com/member/job/levels"
    page = move_url.move_url(page, instance, server, url)
    page = access_create_level(page)

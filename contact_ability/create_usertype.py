...
def fill_field(page):
    ...
    # 사용자 유형 입력 
    page.locator('input.lw_input[placeholder="사용자 유형"]').last.fill(f"준일_자동화_사용자유형_{timestamp}")
...
def access_create_usertype(page) :
    
    # 수정 버튼 선택 
    page.locator('button.lw_btn:text-is("수정")').click()
    
    # 사용자 유형 추가
    page.locator('button.btn_add_row:text-is("사용자 유형 추가")').click()
...
def create_usertype(page, instance, server) :
    url = "admin.worksmobile.com/member/job/user-types"
    page = move_url.move_url(page, instance, server, url)
    page = access_create_usertype(page)
